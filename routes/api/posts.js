...
@dscript Add a comment to a post with id = id
@access Private
*/
router.post('/comment/:id', passport.authenticate('jwt', {session: false}), (req,res) => {
    // We can use the same validation as the creating a post since it has the same fields
    const {errors, isValid} = validatePostInput(req.body);
    ...
    Post.findById(req.params.id)
        .then( post => {
            ...
            // Add to comments array for this post
...
@dscript Delete a comment from post
@access Private
*/
    ...
    Post.findById(req.params.id)
        .then( post => {
            ...
            if(post.comments.filter(comment => comment._id.toString() === req.params.comment_id).length === 0) {
                // If length === 0 means we didnt find the comment in the DB
